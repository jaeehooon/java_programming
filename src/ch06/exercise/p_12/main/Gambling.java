package ch06.exercise.p_12.main;
import ch06.exercise.p_12.user.Person;
...
public class Gambling {
    ...
    public void run() {
        ...
        users = new Person[numUsers];               // Person 객체배열 생성
        scan.nextLine();                            // 버퍼 비우기

        /* 게임에 참여할 선수의 이름 설정 */
        setUsers();                                 // 게임에 참여할 user 세팅
        ...
        while (true) {
            ...
            if (idx >= numUsers - 1)                // 게임 참여자 수 - 1 (인덱스는 0부터 시작하기 때문)
                idx = 0;
            else
                idx++;
        }
    ...
    void setUsers() {
        for(int i=0; i<users.length; i++) {
            ...
            String name = scan.nextLine();                      // 사람 이름 입력
            users[i] = new Person();                            // Person 객체 생성
            users[i].setName(name);                             // 해당 객체의 이름 설정
        }
    }
    ...
    private int[] makeRandomNumbers() {
        ...
        for(int i=0; i<numbers.length; i++)
            numbers[i] = (int)(Math.random() * 3 + 1);      // 1~3 의 숫자 생성
        return numbers;
    }
